...
import { FooterMenuItem } from '@widgets/footer/constants/constants.ts';
...
export const FooterMenu: FC<Props> = ({ className, items }) => {
      ...
      {items.map(item => (
        <div className={styles.footerColumn} key={item.title}>
          ...
          <ul className={styles.footerLinks}>
            {item.links.map(link => (
              <li key={link.text}>
                <Button className={styles.linkBtn} variant={'ghost'} onClick={link.onClick}>
                  {formatLinkText(link.text, link.shortText)}
                </Button>
              </li>
            ))}
          </ul>
