...
const CarouselItemComponent: FC<Props> = ({ data, className, onClick }) => {
  const { t } = useTranslation('slots');

  return (
    <Link to={data.link} className={clsx(styles.carouselItem, className)} draggable={false} onClick={onClick}>
      <div className={clsx(styles.wrapper, data.is_favorite && styles.is_favorite)}>
        <div className={clsx(styles.carouselItemImage)}>
          ...
          {data.blocked_countries && (
            <>
              ...
              <div className={styles.blockedContent}>
                ...
                <p className={styles.blockedText}>{t('messages.blockedInRegion')}</p>
              </div>
            </>
          )}
        </div>
