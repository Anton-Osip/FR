...
export const HeroSlot: FC = () => {
  ...
  const renderCustomPagination = (): ReactElement => {
    ...
    return (
      <div className={styles.slotPagination} role="tablist" aria-label={t('heroSlot.slide')}>
        {dotsBefore.map(idx => (
          <div
            key={idx}
            ...
            role="tab"
            aria-selected={false}
            aria-label={`${t('heroSlot.slide')} ${idx + 1}`}
          />
        ))}

        <div
          className={styles.slotPaginationBar}
          role="tab"
          aria-selected={true}
          aria-label={`${t('heroSlot.slide')} ${activeIndex + 1}`}
        />
        ...
        {dotsAfter.map(targetIndex => (
          <div
            ...
            role="tab"
            aria-selected={false}
            aria-label={`${t('heroSlot.slide')} ${targetIndex + 1}`}
          />
        ))}
      </div>
    );
