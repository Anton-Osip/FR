...
export const TelegramLoginWidget: FC<TelegramLoginWidgetProps> = ({ botName }) => {
  useEffect(() => {
    ...
    // Используем текущий origin для установки куки stel_acid на том же домене
    // Это важно для мультидоменной архитектуры - кука должна устанавливаться там, где находится виджет
    const authUrl = `${window.location.origin}${window.location.pathname}${window.location.search}`;
    ...
    feLog.debug('telegram_login_widget.embedded', { botName, authUrl });
