...
const CANVAS_FONT = `${CANVAS_FONT_SIZE}px sans-serif`;
const CANVAS_FILL_RECT = { x: 125, y: 1, width: 62, height: 20 };
const CANVAS_TEXT = 'skylon_fp';
const CANVAS_TEXT_POS = { x: 2, y: 40 };
const CANVAS_ARC_CENTER = { x: 50, y: 50 };
const CANVAS_ARC_RADIUS = 20;
// eslint-disable-next-line no-magic-numbers
const FULL_CIRCLE = Math.PI * 2;
const RAND_ID_DEFAULT_LENGTH = 16;
const BROWSER_ID_LENGTH = 24;
const USER_AGENT_MOBILE_RE = /Mobile|Android|iPhone|iPad|iPod/i;
const WEBGL_VENDOR_PARAM = 0x1f00;
const WEBGL_RENDERER_PARAM = 0x1f01;
...
function randId(n = RAND_ID_DEFAULT_LENGTH): string {
...
async function canvasFingerprint(): Promise<string> {
  ...
  ctx.fillStyle = '#f60';
  ctx.fillRect(CANVAS_FILL_RECT.x, CANVAS_FILL_RECT.y, CANVAS_FILL_RECT.width, CANVAS_FILL_RECT.height);
  ctx.fillStyle = '#069';
  ctx.fillText(CANVAS_TEXT, CANVAS_TEXT_POS.x, CANVAS_TEXT_POS.y);
  ctx.strokeStyle = '#ff0';
  ctx.arc(CANVAS_ARC_CENTER.x, CANVAS_ARC_CENTER.y, CANVAS_ARC_RADIUS, 0, FULL_CIRCLE);
  ctx.stroke();
...
function webglInfo(): { vendor: string; renderer: string } {
  ...
  const vendor = String(gl.getParameter(WEBGL_VENDOR_PARAM) || '');
  const renderer = String(gl.getParameter(WEBGL_RENDERER_PARAM) || '');
...
function getBrowserId(): string {
  try {
    ...
    if (!v) {
      v = randId(BROWSER_ID_LENGTH);
      localStorage.setItem(k, v);
    }

    return v;
  } catch {
    return randId(BROWSER_ID_LENGTH);
  }
}
...
export async function collectClientProfile(): Promise<ClientProfileBase> {
  ...
  const deviceType: 'mobile' | 'desktop' = USER_AGENT_MOBILE_RE.test(userAgent) || uaMobile ? 'mobile' : 'desktop';
