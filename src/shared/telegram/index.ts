 ...
 */
export function waitForTelegramWebApp(timeout = 2000): Promise<boolean> {
 ...
 */
export function waitForInitData(timeout = 2000, intervalMs = 50): Promise<string> {
  return new Promise(resolve => {
    if (typeof window === 'undefined') {
      resolve('');
