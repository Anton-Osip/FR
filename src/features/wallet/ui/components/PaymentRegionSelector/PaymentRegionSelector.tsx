...
import { MessageQuestionIcon, OkIcon } from '@shared/ui/icons';

import styles from './PaymentRegionSelector.module.scss';

import { DEFAULT_CURRENCY_ITEMS } from '@features/wallet/model/constants';
...
export const PaymentRegionSelector: FC<PaymentRegionSelectorProps> = ({
  items = DEFAULT_CURRENCY_ITEMS,
  ...
  const displayLabel = label ?? t('paymentRegionSelector.label');

  return (
    <div className={styles.dropdownWrapper}>
      {balance && (
        <div className={styles.balanceWrapper}>
          <p className={styles.balanceText}>{t('paymentRegionSelector.yourBalance')}</p>
      ...
      {!balance && (
        <>
          <p className={styles.text}>{displayLabel}</p>
          <MessageQuestionIcon />
        </>
      )}
